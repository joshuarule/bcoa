    ...
    return (
        ...
        >
          ...
          >
            ...
            <ProjectFilter isWindowLarge={ this.props.breakpoint === 'large' } onChange={(val) => this.renderFilterTransition(val)}/>
          </div>
          <ul className={`${this.state.inTransition ? 'inTransition' : '' } bp-1_grid-3col bp-2_grid-4col`}>
            { projects &&
              orderedProjects.filter(project => this.filterProjects(project)).map((project, i) => {
                return (
                  <li key={i}>
                    <article className="workProject">
                      <Link to={ `/projects${project.fields.slug}` }>
                        {project.frontmatter.previewImage.image && 
                          <Img
                            fluid={ project.frontmatter.previewImage.image.childImageSharp.fluid }
